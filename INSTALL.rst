...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/archiefvernietingscomponent.git
       $ cd archiefvernietingscomponent
...
``src/archiefvernietingscomponent/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test archiefvernietingscomponent
...
* ``DB_NAME``: name of the database for the project. Defaults to ``archiefvernietingscomponent``.
* ``DB_USER``: username to connect to the database with. Defaults to ``archiefvernietingscomponent``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``archiefvernietingscomponent``.
...
1. Clone or download the code from `Github`_ in a folder like
   ...
   .. code-block:: bash
       ...
       $ git clone git@github.com:maykinmedia/archiefvernietingscomponent.git
       Cloning into 'archiefvernietingscomponent'...
       ...
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting archiefvernietingscomponent_db_1 ... done
       Starting archiefvernietingscomponent_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f archiefvernietingscomponent_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it archiefvernietingscomponent_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it archiefvernietingscomponent_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/archiefvernietingscomponent/
...
environment variables. See ``src/archiefvernietingscomponent/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t archiefvernietingscomponent
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it archiefvernietingscomponent /app/src/manage.py createsuperuser
...
``src/archiefvernietingscomponent/conf``.
