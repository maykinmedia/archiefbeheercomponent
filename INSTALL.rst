...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/rma.git
       $ cd rma
...
``src/rma/conf/local.py``. You can base this file on the
...
* ``DB_NAME``: name of the database for the project. Defaults to ``rma``.
* ``DB_USER``: username to connect to the database with. Defaults to ``rma``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``rma``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting rma_db_1 ... done
       Starting rma_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it rma_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it rma_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/rma/
...
environment variables. See ``src/rma/conf/docker.py`` for
all settings.
...
.. code-block:: bash
    ...
    $ docker exec -it rma /app/src/manage.py createsuperuser
