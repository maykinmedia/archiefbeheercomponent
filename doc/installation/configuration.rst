...
The RMA supports run-time configuration for maximum flexibility to make it fit your
...
The RMA has a simple permission system, consisting of the following permissions:
...
The RMA admin interface and frontend support logging in through ADFS-backed single sign
on (SSO).
...
The RMA does not store, synchronize or copy case data. All data is retrieved through the
...
.. note:: RMA supports multiple services of the same type.

Catalogi API
------------

The RMA uses the Catalogi API to provide filter options based on "zaaktype".
...
that purpose, the RMA needs access to the Documents API.
...
functioning of the RMA, it needs the following scopes:
...
The RMA only offers cases of which the archive action date has passed, to prevent
destruction of cases before their scheduled archiving. This is annoying for testing
purposes, so the RMA supports specifying the "current date".
