...
1. ``rma-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``rma-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``rma-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``rma-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``rma-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``rma-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``rma-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``rma-pr-isort``: checks that imports are correctly
